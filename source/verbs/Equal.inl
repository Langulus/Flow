...
namespace Langulus::Verbs
{
   ...
   constexpr bool Equal::AvailableFor() noexcept {
      if constexpr (sizeof...(A) == 0) {
         return requires (const T& t, Verb& v) { t.Compare(v); }
             or requires (const T& t, Verb& v) { t.Equal(v); }
         ...
         return requires (const T& t, Verb& v, A... a) { t.Compare(v, a...); }
             or requires (const T& t, Verb& v, A... a) { t.Equal(v, a...); }
   ...
   template<CT::Dense T>
   bool Equal::ExecuteIn(T& context, Verb& verb) {
      static_assert(Equal::AvailableFor<T>(),
   ...
   inline bool Equal::ExecuteDefault(const Many& context, Verb& verb) {
